...
keep if variable == "citizens"

rename GeoID fips
keep fips x2020
drop if x2020 == . 

save "$OUT\data\reduced_YCOM_2020.dta", replace 
...
merge 1:1 fips using "$OUT\data\reduced_YCOM_2020.dta"
keep if _merge == 3
drop _merge 

rename x2020 support_gw_citizens
label var support_gw_citizens "Citizens should do more to address global warming (%)"

save "$OUT\data\first_look_project_data.dta", replace 

* First look
summarize support_gw_citizens TFE // mean: 57.9247 
correlate support_gw_citizens TFE // corr: -0.2059
histogram TFE, bins(50) color("128 100 162")
graph export "$OUT\graphs\histo_TFE.png", as(png) replace

twoway ///
    (scatter support_gw_citizens TFE, msymbol(o) mcolor("128 100 162")) ///
    (lfit support_gw_citizens TFE), ///
...
reg support_gw_citizens TFE, vce(robust)
eststo m0

* Model 1: with controls + state FE, clustered at 60km grid cell
reg support_gw_citizens TFE $geo $hist i.statea, cluster(km_grid_cel_code)
eststo m1
