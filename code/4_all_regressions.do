...
cap mkdir "$OUT\graphs\ystandardized"
...
*******************************************************
* Create mean-centered and standardized outcomes
*******************************************************
...
*******************************************************
* GRAPHS: mean-centered outcome and z-standardized outcome
*******************************************************
...
graph export "$OUT\graphs\ystandardized\scatter_citizensOppose_z.png", replace as(png)
...
graph export "$OUT\graphs\ystandardized\scatter_localofficialsOppose_z.png", replace as(png)
...
graph export "$OUT\graphs\ystandardized\scatter_priorityOppose_z.png", replace as(png)
...
graph export "$OUT\graphs\ystandardized\scatter_reducetaxOppose_z.png", replace as(png)


* ---------- (C) Graphs with  raw outcome variables (untransformed) ---------- 
...
marginsplot, recast(line) noci ///
    title("Citizens should do less to adress global warming") ///
    subtitle("{&beta}{sub:1} = `beta1'*** (raw outcome)") ///
    xtitle("Total Frontier Experience (decades)") ///
    ytitle("Outcome level (p.p.)") ///
    xscale(range(0 6)) ///
    xlabel(0(1)6) ///
    plot1opts(lcolor(black) lwidth(medthick)) ///
...
graph export "$OUT\graphs\normal\scatter_citizensOppose_raw.png", replace as(png)
...
graph export "$OUT\graphs\normal\scatter_localofficialsOppose_raw.png", replace as(png)
...
graph export "$OUT\graphs\normal\scatter_priorityOppose_raw.png", replace as(png)
...
graph export "$OUT\graphs\normal\scatter_reducetaxOppose_raw.png", replace as(png)
